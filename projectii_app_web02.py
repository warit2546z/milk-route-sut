...
# ==========================================
# ฟังก์ชันถอดรหัสเส้นทางของ Google Maps / DistanceMatrix.ai
# ==========================================
...
st.title("🗺️ ระบบวางแผนเส้นทางขนส่งนม (Google/DistanceMatrix Engine)")
st.markdown("วิเคราะห์เส้นทางด้วยสมองกล OR-Tools และแสดงผลด้วยข้อมูลจราจรจากโครงข่าย Google Maps")
...
with st.sidebar:
    ...
    API_KEY = st.text_input("DistanceMatrix / Google API Key", value="", type="password")
...
# ตัวกรองป้องกัน Error จากช่องว่างใน Excel
def safe_float(val):
    try:
        f = float(val)
...
# ==========================================
st.markdown("---")
if st.button("🚀 ประมวลผลและสร้างแผนที่", type="primary", use_container_width=True):
    if not API_KEY:
    ...
    if solution:
        ...
        # ----------------------------------------------------
        # การเรียก API ของ DistanceMatrix.ai (ระบบเทียบเท่า Google)
        # ----------------------------------------------------
        with st.spinner('กำลังเชื่อมต่อเซิร์ฟเวอร์เส้นทางถนนจริง...'):
            ...
            # ✨ อัปเดต: เปลี่ยน URL เป็นเซิร์ฟเวอร์ของ DistanceMatrix.ai
            url = "https://api.distancematrix.ai/maps/api/directions/json"
            params = {
                "origin": origin,
            ...
            if data.get('status') == 'OK':
                ...
                with col_map:
                    ...
                    plugins.AntPath(
                        ...
                        delay=800, dash_array=[15, 30], color="#2980B9", pulse_color="#FFFFFF", weight=6,
                        name='Route (AntPath)'
                    ).add_to(m)
                    ...
                    st.download_button(
                        ...
                        file_name="Optimized_Route_Map.html",
                        mime="text/html",
                ...
                st.error(f"❌ API Error: {data.get('error_message', data.get('status', 'Unknown Error'))}")
        else:
            st.error(f"❌ HTTP Error: ไม่สามารถเชื่อมต่อเซิร์ฟเวอร์ API ได้ (Status Code: {res.status_code})")
    else:
