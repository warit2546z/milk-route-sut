...
import folium
from folium import plugins  # ✨ เพิ่มไลบรารีสำหรับ AntPath
...
st.markdown("วิเคราะห์ประสิทธิภาพโลจิสติกส์ พร้อมระบบ Deep Link สั่งเปิดแผนที่ และแผนที่ทิศทางแบบ AntPath")
    ...
    if solution:
        ...
        if res.status_code == 200:
            ...
            with col_map:
                ...
                # ดึงจุดพิกัดเส้นทางทั้งหมด
                all_points = []
                for leg in route_data['legs']:
                    for p in leg['points']: all_points.append([p['latitude'], p['longitude']])
                
                # ✨ เปลี่ยนจาก PolyLine ปกติ เป็น AntPath (เส้นไหลตามทิศทาง)
                plugins.AntPath(
                    locations=all_points,
                    delay=800,             # ความเร็วในการไหล (ค่าน้อย = เร็ว)
                    dash_array=[15, 30],   # ความยาวของเส้นประและช่องว่าง
                    color="#2980B9",       # สีเส้นหลัก
                    pulse_color="#FFFFFF", # สีของเส้นแอนิเมชันที่วิ่ง
                    weight=6,
                    opacity=0.8,
