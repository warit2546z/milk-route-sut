...
st.markdown("วิเคราะห์เส้นทางอัจฉริยะ พร้อมแผนที่แสดงลำดับการจัดส่งและรายงาน Excel")
...
if st.button("🚀 ประมวลผลเส้นทางและวิเคราะห์เปรียบเทียบ", type="primary", use_container_width=True):
    ...
    if solution:
        ...
        if res.status_code == 200:
            ...
            c3.metric("CO2", f"{(dist_km/KM_L)*EMISSION_FACTOR:.2f} kg", f"{((dist_km - baseline_km)/KM_L)*EMISSION_FACTOR:.2f} kg")

            # --- แผนที่และตาราง ---
            col_map, col_table = st.columns([1.5, 1.5])
            
            with col_map:
                st.subheader("🗺️ แผนที่เส้นทางขนส่ง")
            ...
            with col_table:
                st.subheader("📋 ตารางลำดับคิวงาน")
                schedule = []
                curr_time = datetime.combine(datetime.today(), DEPART_TIME)
                for i, n in enumerate(route_indices[:-1]):
                    if i > 0: curr_time += timedelta(minutes=math.ceil(route_data['legs'][i-1]['summary']['travelTimeInSeconds']/60))
                    schedule.append({"คิว": i, "สถานที่": edited_df.iloc[n]["ชื่อสถานที่"], "เวลาที่ถึง": curr_time.strftime("%H:%M")})
                ...
                st.markdown("---")
                buf = io.BytesIO()
                # 🛠️ ใช้ engine='xlsxwriter' แทนเพื่อลดปัญหา Library ไม่ครบ
                with pd.ExcelWriter(buf, engine='xlsxwriter') as writer:
                    pd.DataFrame(schedule).to_excel(writer, index=False, sheet_name='Sheet1')
                st.download_button("📥 ดาวน์โหลดใบงาน (Excel)", buf.getvalue(), "WorkOrder.xlsx", use_container_width=True)
    else:
