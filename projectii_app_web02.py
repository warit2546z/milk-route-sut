...
if st.button("🚀 ประมวลผลเส้นทางและวิเคราะห์เปรียบเทียบ", type="primary", use_container_width=True):
    ...
    if solution:
        ...
        # ✨ อัปเดต 3: ระบบอ่านพื้นที่ห้ามผ่านแบบหลายกล่อง (Multi-Rectangles)
        rectangles = []
        if AVOID_AREA.strip() != "":
            for line in AVOID_AREA.strip().split('\n'):
                ...
                try:
                    ...
                    pass # หากพิมพ์ผิดบรรทัดไหน ให้ข้ามบรรทัดนั้นไป ไม่ให้แอปพัง
        ...
        if res.status_code == 200:
            ...
            with col_map:
                ...
                # ✨ อัปเดต 4: วาดสี่เหลี่ยมสีแดงหลายๆ กล่องบนแผนที่ตามที่ผู้ใช้กรอก
                if AVOID_AREA.strip() != "":
