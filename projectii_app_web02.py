...
st.markdown("ระบบวิเคราะห์เส้นทางอัจฉริยะ พร้อมการนำทางจริงและฟังก์ชันหลีกเลี่ยงเส้นทางที่ปิดใช้งาน")
...
with st.sidebar:
    ...
    st.caption("รูปแบบ: ลัต,ลองซ้ายบน:ลัต,ลองขวาล่าง")
...
if st.button("🚀 ประมวลผลเส้นทางและวิเคราะห์เปรียบเทียบ", type="primary", use_container_width=True):
    ...
    if solution:
        ...
        # ----------------------------------------------------
        # ✨ อัปเดต: การเรียก API TomTom พร้อมส่งข้อมูลหลีกเลี่ยงถนน
        # ----------------------------------------------------
        ...
        # ถ้ามีการพิมพ์ค่าพื้นที่ห้ามผ่านใน Sidebar ให้ใส่ parameter avoidAreas เพิ่มเข้าไป
        if AVOID_AREA.strip() != "":
            api_params["avoidAreas"] = AVOID_AREA.strip()
        ...
        if res.status_code == 200:
            ...
            with col_map:
                ...
                m = folium.Map(location=coords[0], zoom_start=14) # ซูมเข้าไปอีกนิดให้เห็นชัดๆ
                ...
                # ถ้าระบุพิกัดห้ามผ่าน ให้วาดรูปสี่เหลี่ยมสีแดงลงบนแผนที่ด้วย เพื่อยืนยันว่าบล็อกถูกที่
                if AVOID_AREA.strip() != "":
                    try:
                        tl_str, br_str = AVOID_AREA.strip().split(':')
                        tl_lat, tl_lon = map(float, tl_str.split(','))
                        br_lat, br_lon = map(float, br_str.split(','))
                        folium.Rectangle(
                            bounds=[[tl_lat, tl_lon], [br_lat, br_lon]],
                            color='#E74C3C', fill=True, fill_color='#E74C3C', fill_opacity=0.3,
                            name='พื้นที่ห้ามผ่าน'
                        ).add_to(m)
                    except: pass
