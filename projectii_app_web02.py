...
st.markdown("วิเคราะห์ประสิทธิภาพโลจิสติกส์: ระยะทาง, ต้นทุน, สิ่งแวดล้อม และเวลาปฏิบัติงาน")
...
if st.button("🚀 ประมวลผลเส้นทางและวิเคราะห์เปรียบเทียบ", type="primary", use_container_width=True):
    ...
    if solution:
        ...
        if res.status_code == 200:
            ...
            c1, c2, c3, c4 = st.columns(4) # ปรับเป็น 4 คอลัมน์
            
            # คำนวณเปรียบเทียบ (Delta)
            ...
            # ✨ เพิ่ม: คำนวณเวลาเดินทางรวม (Total Travel Time)
            total_travel_sec = summary['travelTimeInSeconds']
            hh, mm = divmod(total_travel_sec // 60, 60)
            time_display = f"{int(hh)} ชม. {int(mm)} นาที" if hh > 0 else f"{int(mm)} นาที"
            c4.metric("เวลาเดินทางรวม", time_display)
            ...
            with col_table:
                st.subheader("📋 ตารางวิเคราะห์คิวงานละเอียด")
                schedule = []
                ...
                for i, n in enumerate(route_indices[:-1]):
                    ...
                    schedule.append({
                        "คิว": i, "สถานที่": edited_df.iloc[n]["ชื่อสถานที่"], "เวลาที่ถึง": curr_time.strftime("%H:%M"),
                        "เดินทาง (นาที)": t_min if i > 0 else "-", "ระยะทาง (กม.)": f"{l_dist:.2f}" if i > 0 else "-",
                        "น้ำมัน (ลิตร)": f"{f_used:.2f}" if i > 0 else "-", "CO2 (kg)": f"{c_leg:.2f}" if i > 0 else "-"
                    })
                ...
                st.download_button("📥 ดาวน์โหลดใบงาน Excel", buf.getvalue(), "Detailed_MilkRun_Plan.xlsx", use_container_width=True)
    else:
