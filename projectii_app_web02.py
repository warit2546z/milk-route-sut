...
with st.sidebar:
    ...
    # ✨ ฟีเจอร์ใหม่: เลือกประเภทรถเพื่อให้ AI กล้าพาเข้าทางเล็ก
    TRAVEL_MODE = st.selectbox("ประเภทยานพาหนะ (ทดลองเปลี่ยนหากระบบไม่ยอมอ้อมถนน)", ["car", "van", "motorcycle", "truck"]) 
...
if st.button("🚀 ประมวลผลเส้นทางและวิเคราะห์เปรียบเทียบ", type="primary", use_container_width=True):
    ...
    if solution:
        ...
        # ส่ง Travel Mode เข้าไปด้วย เพื่อให้ AI กล้าพาเข้าซอยเล็ก/ทางลัด
        api_params = {"key": API_KEY, "travelMode": TRAVEL_MODE}
        
        if AVOID_AREA.strip() != "":
            try:
                p1_str, p2_str = AVOID_AREA.strip().split(':')
                lat1, lon1 = map(float, p1_str.split(','))
                lat2, lon2 = map(float, p2_str.split(','))
                
                # จัดเรียงมุมให้ถูกต้องสำหรับ TomTom
                ...
                st.error("⚠️ รูปแบบพิกัดพื้นที่ห้ามผ่านไม่ถูกต้อง ระบบจึงค้นหาเส้นทางแบบปกติแทน")
        ...
        if res.status_code == 200:
            ...
            with col_map:
                ...
                # วาดรูปสี่เหลี่ยมสีแดงลงบนแผนที่
                if AVOID_AREA.strip() != "":
                    try:
                        p1_str, p2_str = AVOID_AREA.strip().split(':')
                        lat1, lon1 = map(float, p1_str.split(','))
