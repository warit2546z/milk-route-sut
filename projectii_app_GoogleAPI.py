...
st.markdown("ระบบวิเคราะห์เส้นทางอัจฉริยะ (โหมดทดลอง: ใช้ Google Distance Matrix API 100%)")
...
# เก็บลำการ Haversine ไว้เป็นแผนสำรองฉุกเฉิน
...
if st.button("🚀 ประมวลผลเส้นทางและวิเคราะห์เปรียบเทียบ", type="primary", use_container_width=True):
    ...
    # ✨ โค้ดส่วนที่เปลี่ยนไปใช้ Google Distance Matrix API 100%
    with st.spinner(f'กำลังดึงตารางระยะทางจริงจาก Google Distance Matrix API สำหรับรถ {total_vehicles} คัน...'):
        coords = edited_df[['Lat', 'Lon']].values.tolist()
        num_points = len(coords)
        dist_matrix = [[0] * num_points for _ in range(num_points)]
        
        origins_param = "|".join([f"{c[0]},{c[1]}" for c in coords])
        destinations_param = origins_param
        ...
        try:
            dm_res = requests.get(dm_url, params=dm_params, verify=False)
            if dm_res.status_code == 200:
                dm_data = dm_res.json()
                if dm_data.get('status') == 'OK':
                    for i in range(num_points):
                        row_results = dm_data['rows'][i]['elements']
                        for j in range(num_points):
                            if row_results[j]['status'] == 'OK':
                                dist_matrix[i][j] = row_results[j]['distance']['value']
                            else:
                                dist_matrix[i][j] = haversine_distance(coords[i], coords[j])
                else:
                    st.error(f"❌ Distance Matrix API Error: {dm_data.get('status')} - {dm_data.get('error_message', '')}")
                    st.stop()
            else:
                st.error("❌ ไม่สามารถเชื่อมต่อ Google Distance Matrix API ได้")
                st.stop()
        except Exception as e:
        ...
        def time_callback(from_index, to_index):
            ...
            # แปลงระยะทางเมตรเป็นเวลาแบบคร่าวๆ (30 กม./ชม.) เพื่อใช้ใน OR-Tools Constraint
