...
st.markdown("ระบบวิเคราะห์เส้นทางอัจฉริยะ รองรับรถขนส่งหลายประเภท พร้อมประเมินจราจร Real-time และคาร์บอนฟุตพริ้นท์เชิงลึก")
...
with st.sidebar:
    ...
    c1, c2 = st.columns(2)
    n_pickup = c1.number_input("รถกระบะ (คัน)", 0, 5, 1)
    n_box = c2.number_input("กระบะตู้ทึบ (คัน)", 0, 5, 0)
    n_4w = c1.number_input("บรรทุก 4 ล้อ (คัน)", 0, 5, 0)
    n_6w = c2.number_input("บรรทุก 6 ล้อ (คัน)", 0, 5, 0)
    
    st.markdown("---")
    st.subheader("⛽ อัตราสิ้นเปลืองน้ำมันขณะวิ่ง (km/L)")
    c3, c4 = st.columns(2)
    km_l_pickup = c3.number_input("รถกระบะ", min_value=1.0, value=12.0, step=0.5, format="%.1f")
    km_l_box = c4.number_input("กระบะตู้ทึบ", min_value=1.0, value=10.0, step=0.5, format="%.1f")
    km_l_4w = c3.number_input("บรรทุก 4 ล้อ", min_value=1.0, value=8.0, step=0.5, format="%.1f")
    km_l_6w = c4.number_input("บรรทุก 6 ล้อ", min_value=1.0, value=6.0, step=0.5, format="%.1f")
    
    st.subheader("🛑 อัตราสิ้นเปลืองขณะจอดรถติด (L/ชั่วโมง)")
    c5, c6 = st.columns(2)
    idle_pickup = c5.number_input("รถกระบะ (L/h)", min_value=0.1, value=1.2, step=0.1, format="%.1f")
    idle_box = c6.number_input("กระบะตู้ทึบ (L/h)", min_value=0.1, value=1.5, step=0.1, format="%.1f")
    idle_4w = c5.number_input("บรรทุก 4 ล้อ (L/h)", min_value=0.1, value=2.0, step=0.1, format="%.1f")
    idle_6w = c6.number_input("บรรทุก 6 ล้อ (L/h)", min_value=0.1, value=3.0, step=0.1, format="%.1f")
    
    # ผูกโปรไฟล์รถเข้ากับตัวแปรควบคุม
    VEHICLE_PROFILES = {
        "pickup": {"name": "รถกระบะ", "coolers": 2, "km_l": km_l_pickup, "idle_l_h": idle_pickup},
        "box_pickup": {"name": "กระบะตู้ทึบ", "coolers": 3, "km_l": km_l_box, "idle_l_h": idle_box},
        "truck_4w": {"name": "บรรทุก 4 ล้อ", "coolers": 5, "km_l": km_l_4w, "idle_l_h": idle_4w},
        "truck_6w": {"name": "บรรทุก 6 ล้อ", "coolers": 10, "km_l": km_l_6w, "idle_l_h": idle_6w}
    }
...
# ==========================================
# 4. ประมวลผลและคำนวณเชิงโลจิสติกส์
# ==========================================
...
if st.button("🚀 ประมวลผลเส้นทางแบบผสม (HFVRP)", type="primary", use_container_width=True):
    ...
    if solution:
        ...
        with st.spinner('กำลังคำนวณแบบจำลองเชื้อเพลิงและจราจร Real-time...'):
            ...
            for idx_num, route_obj in enumerate(all_routes):
                v_id = route_obj["v_id"]
                route_indices = route_obj["indices"]
                v_profile = fleet_mix[v_id] 
                ...
                while start_idx < len(route_indices) - 1:
                    ...
                    if res.status_code == 200:
                        ...
                        if data.get('status') == 'OK':
                            ...
                            v_dist_meters += sum([leg['distance']['value'] for leg in route_data['legs']])
                            
                            for leg in route_data['legs']:
                                traffic_time = leg.get('duration_in_traffic', leg.get('duration'))['value']
                                normal_time = leg.get('duration')['value']
                                v_time_seconds += traffic_time
                                v_normal_time_seconds += normal_time
                ...
                # --- การประมวลผลสมการ Dual-State ---
                v_dist_km = v_dist_meters / 1000
                fuel_running = v_dist_km / v_profile['km_l']
                
                delay_seconds = max(0, v_time_seconds - v_normal_time_seconds)
                delay_hours = delay_seconds / 3600
                fuel_idling = delay_hours * v_profile['idle_l_h']
                
                total_fuel_used = fuel_running + fuel_idling
                v_cost = total_fuel_used * THB_L
                v_co2 = total_fuel_used * EMISSION_FACTOR
                ...
                # รีเซ็ตเลขรถลำดับวิ่งจริงให้เรียงกันสวยๆ เป็น 1, 2, 3 บนตารางและแผนที่
                vehicle_map_data.append({
                    "v_display_id": idx_num + 1,
                    "profile": v_profile,
        ...
        if api_success:
            ...
            c3.metric("ต้นทุนน้ำมันรวม (วิ่ง+จอดติด)", f"฿{fleet_total_cost:.2f}")
            ...
            with col_map:
                ...
                for v_data in vehicle_map_data:
                    v_color = color_palette[(v_data["v_display_id"] - 1) % len(color_palette)]
                    ...
                    for idx_pos, n in enumerate(v_data["indices"][1:-1]): 
                        ...
                        folium.Marker(
                            [loc['Lat'], loc['Lon']], 
                            popup=f"[{v_data['profile']['name']}] คันที่ {v_data['v_display_id']} | คิว {idx_pos + 1}: {loc['ชื่อสถานที่']}", 
                            icon=folium.DivIcon(html=icon_html)
                        ).add_to(m)
                st_folium(m, width="100%", height=500, returned_objects=[])

            with col_table:
                st.subheader("📋 ตารางลำดับงาน (แยกรายคัน)")
                buf = io.BytesIO()
                with pd.ExcelWriter(buf, engine='xlsxwriter') as writer:
                    
                    for v_data in vehicle_map_data:
                        v_color = color_palette[(v_data["v_display_id"] - 1) % len(color_palette)]
                        st.markdown(f"#### <span style='color:{v_color};'>🚚 คันที่ {v_data['v_display_id']} : {v_data['profile']['name']}</span>", unsafe_allow_html=True)
                        st.caption(f"ระยะทาง: {v_data['v_dist_km']:.2f} กม. | ค่าน้ำมันรวม: ฿{v_data['v_cost']:.2f} | ปล่อย CO2: {v_data['v_co2']:.2f} kg")
                        ...
                        sheet_name = f'คันที่_{v_data["v_display_id"]}_{v_data["profile"]["name"]}'
                        df_schedule.to_excel(writer, index=False, sheet_name=sheet_name)
                
                st.download_button("📥 ดาวน์โหลดใบงาน Excel (แยก Sheet)", buf.getvalue(), "SUTMR_Traffic_Adjusted_Plan.xlsx", use_container_width=True)
        else:
            st.error(f"❌ เกิดข้อผิดพลาดจาก Google Maps API: {api_error_msg}")
    else:
        st.error("❌ ไม่สามารถจัดเส้นทางได้ โปรดตรวจสอบเงื่อนไขความจุรถหรือปริมาณงาน")
