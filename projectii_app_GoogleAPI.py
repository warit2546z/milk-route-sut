    ...
    try:
        ...
        if res_ptt.status_code == 200:
            ...
            if result_node is not None and result_node.text:
                ...
                oil_options = {}
                target_types = ["ดีเซล", "แก๊สโซฮor์ 91", "แก๊สโซฮอล์ 95"]
...
st.markdown("ระบบวิเคราะห์เส้นทางอัจฉริยะ (โหมดทดลอง: ใช้ Google Routes API v2 รุ่นใหม่ล่าสุดแบบ 100%)")
...
if st.button("🚀 ประมวลผลเส้นทางและวิเคราะห์เปรียบเทียบ", type="primary", use_container_width=True):
    ...
    # 🚀 ส่วนที่อัปเดตใหม่: เชื่อมต่อผ่าน Google Routes API v2 (ระบบใหม่ล่าสุดของ Google ตามภาพแจ้งเตือน)
    with st.spinner(f'กำลังดึงตารางระยะทางจริงจาก Google Routes API (v2) สำหรับรถ {total_vehicles} คัน...'):
        ...
        # จัดโครงสร้างข้อมูลรูปแบบพิกัดตามมาตรฐาน JSON ของระบบเวอร์ชันใหม่
        ...
        try:
            ...
            if res.status_code == 200:
                data = res.json()
                # นำชุดข้อมูลอาร์เรย์แนวยาวมาแกะพิกัดคู่บรรจบและเขียนบันทึกลงตารางเมทริกซ์
                for item in data:
                ...
                st.error(f"❌ Routes API Error: {res.status_code} - โปรดตรวจสอบว่าได้เปิดใช้ 'Routes API' ใน Google Cloud Console หรือยังครับ")
                st.stop()
        except Exception as e:
...
#Algorithms
        ...
        search_params.first_solution_strategy = routing_enums_pb2.FirstSolutionStrategy.PARALLEL_SAVINGS
