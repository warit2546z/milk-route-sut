...
with st.sidebar:
    ...
    # ✨ อัปเดตเมนูตรงนี้: ถอด SWEEP ออกและนำ GLOBAL_CHEAPEST_ARC กลับมา
    first_sol_dict = {
    ...
    # 1. กล่องเลือกสำหรับ First Solution Strategy (ผู้ใช้สามารถปรับเปลี่ยนเล่นได้)
    ...
    # 2. ป้ายกำกับสำหรับ Metaheuristic (ล็อกเป็น GLS ถาวร)
...
if st.button("🚀 ประมวลผลเส้นทางและวิเคราะห์เปรียบเทียบ", type="primary", use_container_width=True):
    ...
    with st.spinner(f'กำลังดึงตารางระยะทางจริงจาก Google Routes API (v2) สำหรับรถ {total_vehicles} คัน...'):
        ...
        # 🎯 พารามิเตอร์สมองกลจะผูกกับชื่อที่เราเลือกจาก Dictionary ทันที
        search_params = pywrapcp.DefaultRoutingSearchParameters()
        search_params.first_solution_strategy = first_sol_dict[selected_first_sol_name]
        
        # 📌 ล็อกอัลกอริทึม Metaheuristic เป็น Guided Local Search ถาวร
