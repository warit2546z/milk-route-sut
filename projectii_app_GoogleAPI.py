...
st.markdown("ระบบวิเคราะห์เส้นทางอัจฉริยะ รองรับรถขนส่งหลายประเภท พร้อมระบบปรับแต่งอัตราสิ้นเปลืองและประเมินจราจร Real-time")
...
with st.sidebar:
    ...
    st.markdown("---")
    st.subheader("⛽ อัตราสิ้นเปลืองน้ำมัน (km/L)")
    c3, c4 = st.columns(2)
    ...
    VEHICLE_PROFILES = {
        "pickup": {"name": "รถกระบะ", "coolers": 2, "km_l": km_l_pickup},
        "box_pickup": {"name": "กระบะตู้ทึบ", "coolers": 3, "km_l": km_l_box},
        "truck_4w": {"name": "บรรทุก 4 ล้อ", "coolers": 5, "km_l": km_l_4w},
        "truck_6w": {"name": "บรรทุก 6 ล้อ", "coolers": 10, "km_l": km_l_6w}
    }
...
# ==========================================
# 4. ประมวลผล (Heterogeneous Optimization)
# ==========================================
...
if st.button("🚀 ประมวลผลเส้นทางแบบผสม (HFVRP)", type="primary", use_container_width=True):
    ...
    if solution:
        ...
        if api_success:
            ...
            # --- ส่วนที่ 2: ✨ อัปเดต ตารางแยกตามคันรถ ---
            with col_table:
                ...
                with pd.ExcelWriter(buf, engine='xlsxwriter') as writer:
                    ...
                    # วนลูปวาดตารางทีละคัน
                    for v_data in vehicle_map_data:
                        v_color = color_palette[(v_data["v_id"] - 1) % len(color_palette)]
                        
                        # สร้างหัวข้อแยกแต่ละคัน พร้อมบอกสรุประยะทางและค่าน้ำมันของคันนั้นๆ
                        st.markdown(f"#### <span style='color:{v_color};'>🚚 คันที่ {v_data['v_id']} : {v_data['profile']['name']}</span>", unsafe_allow_html=True)
                        st.caption(f"ระยะทาง: {v_data['v_dist_km']:.2f} กม. | ค่าน้ำมัน: ฿{v_data['v_cost']:.2f} | ปล่อย CO2: {v_data['v_co2']:.2f} kg")
                        ...
                        # แสดงผล DataFrame บนหน้าเว็บแยกทีละคัน
                        st.dataframe(
                        ...
                        st.markdown("<br>", unsafe_allow_html=True) # เว้นบรรทัดระหว่างตาราง
                        
                        # เซฟข้อมูลตารางของคันนี้ลง Excel Sheet แยกต่างหาก
                        sheet_name = f'คันที่_{v_data["v_id"]}_{v_data["profile"]["name"]}'
                        df_schedule.to_excel(writer, index=False, sheet_name=sheet_name)
                
                # ปุ่มดาวน์โหลดไฟล์ Excel จะรวมทุก Sheet ไว้ในไฟล์เดียว
                st.download_button("📥 ดาวน์โหลดใบงาน Excel (แยก Sheet ตามรถ)", buf.getvalue(), "SUTMR_HFVRP_Plan.xlsx", use_container_width=True)
        else:
