...
with st.sidebar:
    ...
    # ✨ ปรับค่าเริ่มต้น (Default) ให้เป็นน้ำหนักสินค้าที่บรรทุกได้จริง (Net Payload) ถูกต้องตามกฎหมายขนส่ง
...
if st.button("🚀 ประมวลผลเส้นทางและวิเคราะห์เปรียบเทียบ", type="primary", use_container_width=True):
    ...
    if solution:
            ...
            if res.status_code == 200:
                ...
                if gmaps_data.get('status') == 'OK':
                    ...
                    route_results.append({
                        ...
                        'legs': legs,
                        'polyline_points': line_coords if 'line_coords' in locals() else polyline_coords,
                        'indices': indices,
        ...
        st.markdown("---")
        st.subheader("📦 สถานะการบรรทุกน้ำหนักสินค้าของรถแต่ละคัน (Cargo Payload Status)")
