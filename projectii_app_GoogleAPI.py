...
st.markdown("ระบบวิเคราะห์เส้นทางอัจฉริยะ พร้อมหลอดสถานะแสดงน้ำหนักบรรทุกสินค้าจริงของรถแต่ละคัน (Google Maps API)")
...
with st.sidebar:
    ...
    with col6:
        ...
        id_box = st.number_input("ตู้ทึb (L/h)", value=1.5)
            ...
            for rr in route_results:
                ...
                for i, n in enumerate(rr['indices'][:-1]):
                    ...
                    if i > 0:
                        ...
                        # ✨ คำนวณความเร็วเฉลี่ยรายจุด (กม./ชม.) จากเวลาที่ใช้วิ่งจริงรวมสภาพรถติด
                        duration_seconds = leg_data.get('duration_in_traffic', leg_data['duration'])['value']
                        if duration_seconds > 0:
                            speed_kmh = (l_dist) / (duration_seconds / 3600)
                        
                        # ✨ คำนวณการปล่อยก๊าซคาร์บอน CO2 รายจุด (คิดจากอัตราสิ้นเปลืองน้ำมันของแต่ละ Leg)
                        normal_duration = leg_data['duration']['value']
                        leg_delay_sec = max(0, duration_seconds - normal_duration)
                    ...
                    schedule.append({
                        ...
                        "ความเร็วเฉลี่ย (กม./ชม.)": f"{speed_kmh:.1f}" if i > 0 else "-", # คอลัมน์ที่เพิ่มใหม่
                        "คาร์บอน CO2 (kg)": f"{co2_leg:.2f}" if i > 0 else "-",          # คอลัมน์ที่เพิ่มใหม่
