...
# ฟังก์ชันสร้างไฟล์ GPX สำหรับบันทึกแทร็กเส้นทางไปใช้ร่วมกับตัวเครื่องนำทางหรือสมาร์ทโฟน
...
# ฟังก์ชันสร้างไฟล์ GeoJSON สำหรับใช้งานกับ QGIS หรือระบบ GIS ทั่วไป
def generate_geojson(route_results, coords, df):
    features = []

    # 1. สร้างหมุด (Points)
    for i, coord in enumerate(coords):
        loc_name = df.iloc[i].get('ชื่อสถานที่', f"Node {i}")
        point_feature = {
            "type": "Feature",
            "geometry": {
                "type": "Point",
            ...
            "properties": {
                "node_id": i,
                "name": loc_name,
                "type": "Depot" if i == 0 else "Customer/Farm"
            }
        }
        features.append(point_feature)

    # 2. สร้างเส้นทาง (LineString)
    for rr in route_results:
        route_coords = [[lon, lat] for lat, lon in rr['polyline_points']]
        if route_coords:
            line_feature = {
                "type": "Feature",
                "geometry": {
                ...
                "properties": {
                    ...
                    "stroke": rr['color'],
                    "stroke-width": 4,
                    "loaded_weight": rr['loaded_weight'],
                    "vehicle_type": rr['v_info']['type']
                }
            }
    ...
    return json.dumps(geojson_data, ensure_ascii=False, indent=4)

# ฟังก์ชัน Haversine สำรองกรณีหาเส้นทางจริงไม่เจอ
...
st.markdown("แอปพลิเคชันเพื่อการวางแผนขนส่งสินค้าประเภทนม (Standard CVRP Model)")
...
with st.sidebar:
    ...
    # ==========================================
    # 🧠 การตั้งค่าสมองกล (Optimization Sandbox)
    # ==========================================
    st.markdown("---")
    ...
    selected_first_sol_name = st.selectbox(
        "1. อัลกอริทึมร่างเส้นทางตั้งต้น", 
        list(first_sol_dict.keys()),
        index=2 
    )
    ...
    # ถอด DEAD_SPACE_RATIO ออก เพื่อให้สอดคล้องกับงานวิจัย 1D CVRP ทั่วไป
...
if st.button("🚀 ประมวลผลเส้นทางและวิเคราะห์เปรียบเทียบ", type="primary", use_container_width=True):
    ...
    for i, row in edited_df.iterrows():
        ...
        # คำนวณ Gross Weight (น้ำหนักสุทธิ + ภาชนะ)
        ...
        # ไม่มีการคูณเผื่อ Dead Space แล้ว ใช้พิกัดน้ำหนักตรงๆ ตาม CVRP Standard
        demands.append(math.ceil(total_weight_kg))
    ...
    if solution:
        ...
        if route_results:
            ...
            with col_map:
                ...
                for rr in route_results:
                    ...
                    for step, n in enumerate(rr['indices'][1:-1]):
                        ...
                        folium.Marker([loc['Lat'], loc['Lon']], popup=f"{rr['car_name']} | ลำดับ: {step+1}<br>{loc.get('ชื่อสถานที่', '')}", icon=folium.DivIcon(html=icon_html)).add_to(m)
            ...
            with col_table:
                ...
                for rr in route_results:
                    ...
                    for i, n in enumerate(rr['indices'][:-1]):
                        ...
                        row_data = {
                            ...
                            "สถานที่": loc_data.get("ชื่อสถานที่", "-") if i > 0 else "ออกเดินทาง (ฟาร์ม)", 
                ...
                # แบ่งปุ่มดาวน์โหลดเป็น 4 คอลัมน์ และเพิ่มปุ่มดาวน์โหลด .GeoJSON 
                st.write("---")
                ...
                with dl_col4:
                    geojson_data = generate_geojson(route_results, coords, edited_df)
                    st.download_button("🌐 ดาวน์โหลดเส้นทาง (GeoJSON)", geojson_data, "MilkRun_Routes.geojson", mime="application/geo+json", use_container_width=True)
