...
# ✨ ฟังก์ชันสร้างไฟล์ KML สำหรับ Export เส้นทาง
...
st.markdown("ระบบวิเคราะห์เส้นทางอัจฉริยะ พร้อมแบบจำลองประเมิน CO2 เรียลไทม์ (Google Maps Engine)")
...
if st.button("🚀 ประมวลผลเส้นทางและวิเคราะห์เปรียบเทียบ", type="primary", use_container_width=True):
    ...
    if solution:
                ...
                if gmaps_data.get('status') == 'OK':
                    ...
                    for leg in legs:
                        for step in leg['steps']:
                            ...
                            polyline_coords.extend(step_coords)
        ...
        # 🗺️ แบ่งหน้าจอ แผนที่ซ้าย - ตารางคิวงานขวา
        ...
        # 📋 ตารางคิวงาน (แยกตามรถแต่ละคัน)
        with col_table:
            st.subheader("📋 ตารางวิเคราะห์ลำดับคิวงาน (แยกรายคัน)")
            
            # ตัวแปรสำหรับเก็บข้อมูลรวมไว้สร้างไฟล์ Excel ดาวน์โหลด
            all_schedules_for_excel = []
            
            # ✨ วนลูปสร้างตารางทีละคัน
            for rr in route_results:
                # แสดงชื่อรถเป็นหัวข้อย่อย
                ...
                for i, n in enumerate(rr['indices'][:-1]):
                    ...
                    # ข้อมูลรายบรรทัดสำหรับแสดงในเว็บ (ไม่มีคอลัมน์ชื่อรถซ้ำซ้อน)
                    row_data = {
                    ...
                    # ข้อมูลสำหรับ Excel รวม (ต้องมีชื่อรถกำกับ)
                ...
                # วาดตารางสำหรับรถคันนี้ลงบนหน้าเว็บ
                ...
                st.write("") # เว้นบรรทัดระหว่างตารางรถแต่ละคัน

            st.write("---")
            # ✨ ส่วนของปุ่มดาวน์โหลด (นำข้อมูลรวมทั้งหมดมาสร้าง Excel)
