...
if st.button("🚀 ประมวลผลเส้นทางและวิเคราะห์เปรียบเทียบ", type="primary", use_container_width=True):
    ...
    if solution:
        ...
        for idx, route in enumerate(all_routes):
            ...
            if res.status_code == 200:
                ...
                if gmaps_data.get('status') == 'OK':
                    ...
                    # ✨ ดึงพิกัดแบบความละเอียดสูง (Step-by-step) แก้ปัญหาเส้นตัดทุ่ง
                    polyline_coords = []
                    for leg in legs:
        ...
        with col_map:
            ...
            # ✨ บังคับใช้แผนที่ฐาน (Basemap) เป็นของ Google Maps เพื่อให้เส้นเกาะถนนเป๊ะ
            folium.TileLayer(
            ...
            for rr in route_results:
                ...
                for i, n in enumerate(rr['indices'][:-1]):
                    ...
                    schedule.append({
                        ...
                        "สภาพจราจรรายจุด": traffic_status,
                        "เวลาที่เสียจากรถติด (นาที)": f"{delay_min:.1f} นาที" if i > 0 else "-",
