...
def generate_gpx(route_results):
    gpx_header = '<?xml version="1.0" encoding="UTF-8"?>\n<gpx version="1.1" creator="Milk Run Optimization App" xmlns="http://www.topografix.com/GPX/1/1">\n'
    gpx_footer = '</gpx>'
    gpx_body = ""
    for rr in route_results:
        gpx_body += f'  <trk>\n    <name>เส้นทาง {rr["car_name"]}</name>\n    <trkseg>\n'
        for lat, lon in rr['polyline_points']:
            gpx_body += f'      <trkpt lat="{lat}" lon="{lon}"/>\n'
        gpx_body += '    </trkseg>\n  </trk>\n'
...
st.title("🚚 SUT Daily Route Plannimg")
...
with st.sidebar:
    ...
    st.markdown("---")
    # ⚡ เพิ่มส่วนตั้งค่าพลังงาน EV
    st.header("⚡ พลังงานไฟฟ้า (EV)")
    THB_KWH = st.number_input("เรทการชาร์จไฟฟ้า (บาท/kWh)", value=5.5, step=0.5, format="%.2f")
    EF_EV = st.number_input("ค่า EF ไฟฟ้า (kgCO2eq/kWh)", value=0.5562, format="%.4f")

    st.header("⛽ ราคาน้ำมัน (ICE)")
    ...
    # ✨ เพิ่มช่องให้กรอกจำนวนรถ EV
    col1, col2, col_ev = st.columns(3)
    with col1:
        num_pickup = st.number_input("รถกระบะ", min_value=0, value=0, step=1)
        num_4w = st.number_input("บรรทุก 4 ล้อ", min_value=0, value=0, step=1)
    with col2:
        num_box = st.number_input("กระบะตู้ทึบ", min_value=0, value=1, step=1)
        num_6w = st.number_input("บรรทุก 6 ล้อ", min_value=0, value=0, step=1)
    with col_ev:
        num_ev = st.number_input("ตู้ทึบ EV 🌱", min_value=0, value=0, step=1)

    st.markdown("**⚖️ น้ำหนักสินค้าสูงสุดที่บรรทุกได้จริง (kg)**")
    c_w1, c_w2, c_wev = st.columns(3)
    with c_w1:
        cap_pickup = st.number_input("กระบะ", min_value=100, value=1000, step=100)
        cap_4w = st.number_input("4 ล้อ", min_value=100, value=2200, step=100)
    with c_w2:
        cap_box = st.number_input("ตู้ทึบ", min_value=100, value=1500, step=100)
        cap_6w = st.number_input("6 ล้อ", min_value=500, value=9000, step=500)
    with c_wev:
        cap_ev = st.number_input("ตู้ทึบ EV", min_value=100, value=1500, step=100)

    st.markdown("**⛽ อัตราสิ้นเปลืองวิ่ง / จอดติด (ICE: L, EV: kWh)**")
    c_f1, c_f2, c_fev = st.columns(3)
    with c_f1:
    ...
    with c_fev:
        # ✨ ตั้งค่า EV ตามรูปเป๊ะๆ
        km_ev = st.number_input("EV(km/kWh)", value=4.0)
        id_ev = st.number_input("EV(kWh/h)", value=1.5)
    ...
    first_sol_dict = {
        ...
        "GLOBAL_CHEAPEST_ARC (เชื่อมจุดใกล้กันทั้งกระดาน)": routing_enums_pb2.FirstSolutionStrategy.GLOBAL_CHEAPEST_ARC
    }
    selected_first_sol_name = st.selectbox("1. อัลกอริทึมร่างเส้นทางตั้งต้น", list(first_sol_dict.keys()), index=2)
    st.markdown("**2. สมองกลปรับปรุงเส้นทาง**")
    st.success("🧠 GUIDED_LOCAL_SEARCH (เปิดใช้งานถาวร)")

    # ✨ สร้าง List กองทัพรถ (ผสม EV ได้)
    active_vehicles = []
    for _ in range(num_pickup): active_vehicles.append({'type': 'รถกระบะ', 'km_rate': km_pickup, 'idle_rate': id_pickup, 'max_weight': cap_pickup, 'is_ev': False})
    for _ in range(num_box): active_vehicles.append({'type': 'กระบะตู้ทึบ', 'km_rate': km_box, 'idle_rate': id_box, 'max_weight': cap_box, 'is_ev': False})
    # ใช้ค่าเดียวกัน 4w และ 6w แบบเร็วๆ ถ้ามีจำนวน
    for _ in range(num_4w): active_vehicles.append({'type': 'บรรทุก 4 ล้อ', 'km_rate': 8.0, 'idle_rate': 2.0, 'max_weight': cap_4w, 'is_ev': False})
    for _ in range(num_6w): active_vehicles.append({'type': 'บรรทุก 6 ล้อ', 'km_rate': 6.0, 'idle_rate': 2.5, 'max_weight': cap_6w, 'is_ev': False})
    # ใส่ EV เข้าไปในระบบ
    for _ in range(num_ev): active_vehicles.append({'type': 'ตู้ทึบ EV 🌱', 'km_rate': km_ev, 'idle_rate': id_ev, 'max_weight': cap_ev, 'is_ev': True})

    DEAD_SPACE_RATIO = 0.15 
    EMISSION_FACTOR_ICE = 2.70757206 
...
if st.button("🚀 ประมวลผลเส้นทางและวิเคราะห์เปรียบเทียบ", type="primary", use_container_width=True):
    ...
    with st.spinner(f'กำลังดึงตารางระยะทางจริงจาก Google Routes API (v2) สำหรับรถ {total_vehicles} คัน...'):
        ...
        waypoints = [{"waypoint": {"location": {"latLng": {"latitude": c[0], "longitude": c[1]}}}} for c in coords]
        payload = {
            "origins": waypoints,
        ...
        try:
            res = requests.post(routes_url, headers=headers, json=payload, verify=False)
            if res.status_code == 200:
                data = res.json()
                for item in data:
                    ...
                    if i is not None and j is not None:
                        if 'distanceMeters' in item:
                            dist_matrix[i][j] = item['distanceMeters']
                        elif i == j:
                            dist_matrix[i][j] = 0
                        else:
                            dist_matrix[i][j] = haversine_distance(coords[i], coords[j])
            else:
                st.error(f"❌ Routes API Error: {res.status_code} - โปรดตรวจสอบการจำกัดสิทธิ์ API Key ครับ")
                st.stop()
        except Exception as e:
            st.error(f"❌ เกิดข้อผิดพลาดทางระบบเครือข่าย: {e}")
            st.stop()
        ...
        search_params.first_solution_strategy = first_sol_dict[selected_first_sol_name]
    ...
    if solution:
        ...
        for idx, route in enumerate(all_routes):
            ...
            if api_success and all_legs:
                ...
                # ✨ แยกการคำนวณต้นทุนระหว่าง EV และ ICE
                if is_ev:
                    energy_running = dist_km / v_info['km_rate']
                    energy_idling = (traffic_delay_sec / 3600) * v_info['idle_rate']
                    total_energy = energy_running + energy_idling
                    cost_thb = total_energy * THB_KWH
                    co2_kg = total_energy * EF_EV
                else:
                    fuel_running = dist_km / v_info['km_rate']
                    fuel_idling = (traffic_delay_sec / 3600) * v_info['idle_rate']
                    total_energy = fuel_running + fuel_idling
                    cost_thb = total_energy * THB_L
                    co2_kg = total_energy * EMISSION_FACTOR_ICE
                ...
                route_results.append({
                    ...
                    'indices': indices,
                    'color': '#27AE60' if is_ev else map_colors[idx % len(map_colors)], # สีเขียวเด่นๆ ถ้าเป็น EV
                    'v_info': v_info,
        ...
        if route_results:
            ...
            c2.metric("ต้นทุนพลังงานรวม", f"฿{total_cost_thb:.2f}")
            ...
            with col_table:
                ...
                for rr in route_results:
                    ...
                    for i, n in enumerate(rr['indices'][:-1]):
                        ...
                        if i > 0:
                            ...
                            # ✨ ประเมิน CO2 แยกตามประเภทพลังงาน (EV vs ICE)
                            if is_ev:
                                f_run = l_dist / v_info['km_rate']
                                f_idle = (leg_delay_sec / 3600) * v_info['idle_rate']
                                co2_leg = (f_run + f_idle) * EF_EV
                            else:
                                f_run = l_dist / v_info['km_rate']
                                f_idle = (leg_delay_sec / 3600) * v_info['idle_rate']
                                co2_leg = (f_run + f_idle) * EMISSION_FACTOR_ICE
                ...
                with dl_col3:
                    gpx_data = generate_gpx(route_results)
                    st.download_button("📍 ดาวน์โหลดเส้นทาง (GPX)", gpx_data, "MilkRun_Routes.gpx", mime="application/gpx+xml", use_container_width=True)
