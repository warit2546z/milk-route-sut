...
st.markdown("ระบบวิเคราะห์เส้นทางอัจฉริยะ พร้อมหลอดสถานะแสดงน้ำหนักบรรทุกของรถแต่ละคัน (Google Maps API)")
...
with st.sidebar:
    ...
    st.markdown("**⚖️ น้ำหนักบรรทุกสูงสุด (kg) ต่อคัน**")
    col3, col4 = st.columns(2)
    with col3:
        cap_pickup = st.number_input("รถกระบะ", min_value=500, value=1000, step=100, key="cap_p")
        cap_4w = st.number_input("บรรทุก 4 ล้อ", min_value=1000, value=2500, step=100, key="cap_4")
    with col4:
        cap_box = st.number_input("กระบะตู้ทึบ", min_value=500, value=1500, step=100, key="cap_b")
        cap_6w = st.number_input("บรรทุก 6 ล้อ", min_value=2000, value=5000, step=100, key="cap_6")
...
if st.button("🚀 ประมวลผลเส้นทางและวิเคราะห์เปรียบเทียบ", type="primary", use_container_width=True):
    ...
    if sum(demands) > total_fleet_capacity:
        st.error(f"❌ น้ำหนักของรวม ({sum(demands)} kg) เกินความจุของรถทั้งหมดในลาน ({total_fleet_capacity} kg)")
        st.stop()
    ...
    if solution:
        ...
        for idx, route in enumerate(all_routes):
            ...
            gmaps_params = {
                ...
                "mode": "driving", # ล็อคค่าเป็น driving สำหรับ Google Maps
                "departure_time": "now",
                "key": API_KEY
            }
            if waypoints_param:
            ...
            if res.status_code == 200:
                ...
                if gmaps_data.get('status') == 'OK':
                    ...
                    st.error(f"❌ Google Maps Error: {gmaps_data.get('status')} - {gmaps_data.get('error_message', '')}")
            else:
        ...
        st.markdown("---")
        st.subheader("📦 สถานะการบรรทุกน้ำหนักของรถแต่ละคัน (Payload Status)")
        ...
        with col_table:
            st.subheader("📋 ตารางวิเคราะห์ลำดับคิวงาน")
            schedule = []
            ...
            for rr in route_results:
                ...
                for i, n in enumerate(rr['indices'][:-1]):
                    ...
                    schedule.append({
                        "คันที่": rr['car_name'],
                        "สถานที่": loc_data["ชื่อสถานที่"] if i > 0 else "ออกเดินทาง", 
            ...
            st.download_button("📥 ดาวน์โหลดใบงานรวม (Excel)", buf.getvalue(), "MilkRun_Google_Plan.xlsx", use_container_width=True)
