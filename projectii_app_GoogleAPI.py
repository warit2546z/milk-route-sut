...
# ==========================================
# ฟังก์ชันดึงราคาน้ำมัน Real-time (มีแผนสำรอง PTT OR)
# ==========================================
...
def fetch_today_oil_price():
    ...
    try:
        ...
        pass # ถ้า API หลักล่ม ปล่อยผ่านไปทำแผน B
    ...
    try:
        ...
        res_ptt = requests.post(url_ptt, data=body, headers=headers, timeout=5)
        if res_ptt.status_code == 200:
            ...
            if result_node is not None and result_node.text:
                ...
                if oil_options:
                    now_str = datetime.now().strftime("%Y-%m-%d (ต่อตรง PTT API)")
...
# สมการ Haversine Formula (Sinnott, 1984) สำหรับประเมินระยะทางตั้งต้น
...
if st.button("🚀 ประมวลผลเส้นทางและวิเคราะห์เปรียบเทียบ", type="primary", use_container_width=True):
    ...
    # Data Cleaning ป้องกันค่า NaN หรือช่องว่าง
